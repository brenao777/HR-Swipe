...
export const submitHandler = createAsyncThunk('user/submitHandler', async (formData) => {
  const response = await axiosInstance.post<AuthResponse>('/auth/register', formData);
  setAccessToken(response.data.accessToken);
  return response.data;
});

export const loginHandler = createAsyncThunk('user/loginHandler', async (formData) => {
